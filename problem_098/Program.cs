...
internal static class Program
{
    ...
    private static string LoadData()
    {
        ...
        _cachedData = File.ReadAllText("p098_words.txt");
        return _cachedData;
    }
        ...
        foreach (var group in anagramGroups.Values)
        {
            ...
            for (int j = i + 1; j < group.Count; j++)
            {
                ...
                foreach (long sq in squares)
                {
                    ...
                    {
                        int li = w1[k] - 'A'; int di = digits[k];
                        if (letterToDigit[li] == -1 && digitToLetter[di] == -1)
                        { letterToDigit[li] = di; digitToLetter[di] = li; }
    ...
    static void Main() => Bench.Run(98, Solve);
}
