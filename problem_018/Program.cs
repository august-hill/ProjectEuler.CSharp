...
class TNode
{
    ...
    public TNode(int value) { Value = value; }
}
...
internal static class Program
{
    ...
    static int Summation(int n) => n * (n + 1) / 2;

    private static int WalkPath(TNode root, BitArray path)
    {
        int sum = root.Value;
        for (int i = 0; i < path.Length; i++)
        {
            root = path[i] ? root.RChild : root.LChild;
            sum += root.Value;
        }
        return sum;
    }
    ...
    static long Solve()
    {
        ...
        {
            if (Summation(i) == nodes) { levels = i; break; }
        }
        ...
        BitArray pathBits = new BitArray(levels - 1);
        int greatestSum = 0;
        for (int i = 0; i < n; i++)
        {
            int k = i;
            ...
            {
                pathBits.Set(j, (k & 1) == 1);
                k >>= 1;
            }
            int sum = WalkPath(root, pathBits);
            if (sum > greatestSum) greatestSum = sum;
        }
        return greatestSum;
    }

    static void Main() => Bench.Run(18, Solve);
}
